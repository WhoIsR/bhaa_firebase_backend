...
func SetupRouter() *gin.Engine {
	// gin.Default() udah include otomatis fitur Logger & Recovery biar nggak gampang crash
	r := gin.Default()

	// CORS Middleware (Penting banget biar request dari Flutter/Postman nggak diblokir)
	...
	// Panggil pelayan (handlers) yang udah lu bikin di Step 8
	authHandler := handlers.NewAuthHandler()
	productHandler := handlers.NewProductHandler()

	// Kita bungkus semua rute di dalam grup "/v1" (Versi 1)
	v1 := r.Group("/v1")
	...
	// - Jalur Admin Khusus (Buat Nambah, Ngedit, Ngapus)
